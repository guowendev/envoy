...
namespace Envoy {
	...
	MainCommonBase::MainCommonBase(const OptionsImpl& options, Event::TimeSystem& time_system,
			...
			std::unique_ptr<ProcessContext> process_context) : options_(options), component_factory_(component_factory), thread_factory_(thread_factory),
		file_system_(file_system), symbol_table_(Stats::SymbolTableCreator::initAndMakeSymbolTable(options_.fakeSymbolTableEnabled())), stats_allocator_(*symbol_table_) {
			switch (options_.mode()) {
				...
				case Server::Mode::Serve:
					{
						...
						logging_context_ = std::make_unique<Logger::Context>(options_.logLevel(), options_.logFormat(), log_lock);
				...
				case Server::Mode::Validate:
					restarter_ = std::make_unique<Server::HotRestartNopImpl>();
					logging_context_ = std::make_unique<Logger::Context>(options_.logLevel(), options_.logFormat(), restarter_->logLock());
					break;
			}
		}
