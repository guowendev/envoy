...
namespace Envoy {
	...
	class ProdComponentFactory : public Server::ComponentFactory {
		public:
			...
			Runtime::LoaderPtr createRuntime(Server::Instance& server, Server::Configuration::Initial& config) override;
	};
	...
	class MainCommonBase {
		public:
			...
			using AdminRequestFn = std::function<void(const Http::HeaderMap& response_headers, absl::string_view body)>;
